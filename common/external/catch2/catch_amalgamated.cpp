...
Clara::Parser makeCommandLineParser(ConfigData& config)
{
    ...
    auto const setBenchmarkSamples = [&](std::string const& samples) {
        ...
        if (!parsedSamples) {
            return ParserResult::runtimeError("Could not parse '" + samples + "' as benchmark samples");
        }
        if (*parsedSamples == 0) {
            return ParserResult::runtimeError(
    ...
    namespace {
        struct GeneratorTracker final : TestCaseTracking::TrackerBase,
            ...
            void close() override
            {
                ...
                if (should_wait_for_child || (m_runState == CompletedSuccessfully && !m_isFiltered // filtered generators cannot meaningfully move
...
void RunContext::assertionEnded(AssertionResult&& result)
{
    ...
    if (result.getResultType() == ResultWas::Ok) {
        ...
        if (result.isOk()) {
                       ...
                       .okToFail()) { // Read from a shared state established
            // before the threads could start, this is
            // fine
...
namespace Matchers {
    namespace Detail {
        ...
        enum class FloatingPointKind : uint8_t { Float,
            Double };
...
enum class Justification : uint8_t { Left,
    Right };
    ...
    Verbosity verbosity)
{
    ...
    for (auto const& desc : descriptions) {
        if (verbosity == Verbosity::Quiet) {
            ...
            out << TextFlow::Column(desc.name + ':').indent(2).width(5 + maxNameLen) + TextFlow::Column(desc.description).initialIndent(0).indent(2).width(CATCH_CONFIG_CONSOLE_WIDTH - maxNameLen - 8)
                << '\n';
        }
    }
    out << '\n'
    ...
    bool isFiltered, Verbosity verbosity)
{
    ...
    if (!tags.empty()) {
        const auto maxTagCount = std::max_element(tags.begin(), tags.end(),
