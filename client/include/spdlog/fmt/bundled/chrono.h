...
auto duration_cast(std::chrono::duration<FromRep, FromPeriod> from) -> To
{
    ...
    if (const_check(factor::den != 1))
        count /= factor::den;
    int ec = 0;
    auto to = To(safe_duration_cast::lossless_integral_conversion<typename To::rep>(
        count, ec));
    if (ec)
        throw_duration_error();
    return to;
...
{
    constexpr auto num_fractional_digits = count_fractional_digits<Duration::period::num,
    ...
    int num_fractional_digits = -1)
{
    ...
    if (num_fractional_digits < 0) {
        ...
        using namespace std;
        num_fractional_digits = count_fractional_digits<Duration::period::num,
            Duration::period::den>::value;
